...
- Trust-list review-window semantics (section 9a) — depends on the tier-wording/duration decision the design doc already flagged as open; the `tag_status` column doesn't need that settled first, but its behavior does. **2026-07-18: discussed with Tim and deliberately tabled, not dropped.** The gap it closes is real — right now, tagging an already-active person is instant and unconditional (`photo_persons`/`memory_persons` writes go straight to `confirmed`, no review of any kind), so anyone in the family, however loosely trusted, can attach content to another member's own profile with zero recourse for that person. That's a real inconsistency against this product's own "you control what happens to you" positioning (same self-write-only pattern as `privacy_tier` and voice consent). Reason for tabling anyway: it's the one privacy control missing today, but it's also genuinely deferrable — it doesn't block the core loop, it's cheap to add later (one column, one filter, one trust-list check), and whether it's actually needed is best learned from real families hitting the awkward-tag scenario post-launch rather than built speculatively now. Revisit once there's real usage, or sooner if an actual complaint surfaces.
