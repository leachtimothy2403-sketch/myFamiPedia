...
- **Tier 1 is retired (migration 025, 2026-07-18).** It used to mean "skip review entirely, auto-submit" — not possible anymore since nothing auto-submits without a human tap, and it had become indistinguishable in practice from not being tier 2 or tier 3. `persons.privacy_tier` now only accepts `2 | 3` (DB CHECK constraint and `collection.routes.ts`'s validation both updated; any existing `1` rows were backfilled to `2`). This is a narrow cleanup, not the redefinition `docs/family_administrator_and_privacy_model.md` section 7 proposes (which would repurpose the *remaining* values entirely for the trust-list tag-review window, unrelated to today's review-card/manual-nudge meaning) — that redefinition still isn't built, and still depends on the trust-list feature itself, which was discussed and deliberately tabled the same day. Don't read "tier 1 retired" as "the section 7 redefinition happened" — it hasn't; tiers 2 and 3 mean exactly what they meant before this cleanup.
